% title page
\linespread{1.0}\selectfont
\thispagestyle{empty}
\pagenumbering{gobble}
\selectlanguage{ngerman}
\pdfbookmark[1]{Titlepage}{Titlepage}
\renewcommand{\headrulewidth}{0pt}
\input{content/title.tex}

%Abstract
\newpage
\thispagestyle{empty}
\input{content/abstract.tex}

% contents page
\newpage
%\pagenumbering{Roman}
%\setcounter{page}{1}
\thispagestyle{fancy}
\fancyhead{}
\fancyfoot{}
\fancyfoot[RO,LE]{\slshape{\thepage}}
\selectlanguage{english}
\pdfbookmark[1]{Contents}{Contents}
\addtocontents{toc}{~\hfill\textbf{Page}\par}
\tableofcontents

% begin thesis content
%\pageshift
\linespread{\textlinespread}\selectfont
\fancyfoot[RO,LE]{\slshape{\thepage}}
\fancyhead[LE]{\slshape{\leftmark}}
\fancyhead[RO]{\slshape{\rightmark}}
\renewcommand{\headrulewidth}{0.5pt}

% introduction
\newpage
\pagenumbering{arabic}
\setcounter{page}{1}
\thispagestyle{plain}
\input{content/introduction.tex}

% theory
\newpage
\thispagestyle{plain}
\input{content/theory.tex}

% preperations
\newpage
\thispagestyle{plain}
\input{content/preperations.tex}

% deep learning
\clearpage
\thispagestyle{plain}
\input{content/deeplearning.tex}

% other sections go here

\newpage
\thispagestyle{plain}
\input{content/results.tex}

% conclusion
\newpage
\thispagestyle{plain}
\input{content/conclusion.tex}

% \newpage
% \thispagestyle{plain}
% \input{content/appendix.tex}

% appendix
\begin{appendix}
	% appendix sections should look like this
	\pageshift
	\thispagestyle{plain}
	\input{content/appendix.tex}
	% Section{Foo}\label{sec:apx_foo}
	% Foo.
\end{appendix}


% bibliography
\linespread{1.0}\selectfont
\newpage
\pagestyle{plain}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\refname}{Bibliography}
\phantomsection
\addcontentsline{toc}{section}{Bibliography}
% Add additional bib files here in the command below, lack of comma is important
\bibliography{bib/experimental,bib/theory}


% finalize thesis content
\renewcommand{\headrulewidth}{0pt}
\fancyhead{}
\fancyfoot{}

% acknowledgment
% \newpage
% \thispagestyle{plain}
% \selectlanguage{ngerman}
% \input{content/acknowledgment.tex}

% \includepdf[pages=-]{documents/Eid_Versicherung.pdf}